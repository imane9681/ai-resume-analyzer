    ...
    <div className="min-h-screen p-3 sm:p-4 md:p-8 pt-20 sm:pt-20" dir={dir}>
      ...
      <div className="max-w-7xl mx-auto mt-8 sm:mt-12">
        ...
        <div className="text-center mb-8 sm:mb-12">
          ...
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold bg-gradient-to-r from-green-600 via-emerald-500 to-green-600 bg-clip-text text-transparent mb-2 sm:mb-4 px-2 leading-normal">
            {t("title")}
          </h1>
              ...
              <div className="p-4 sm:p-6 md:p-8 space-y-5 sm:space-y-8">
                ...
                <div className="bg-gradient-to-br from-green-50 via-emerald-50 to-green-50 dark:from-green-900/20 dark:via-emerald-900/20 dark:to-green-900/20 rounded-xl sm:rounded-2xl p-4 sm:p-6">
                  <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-4">
                   ...
                   <div className={`${isMobile ? 'flex flex-col items-center gap-4' : 'flex flex-row items-center gap-6'}`}>
                     <div className={`relative ${isMobile ? 'w-20 h-20' : 'w-32 h-32'}`}>
                       <svg className={`${isMobile ? 'w-20 h-20' : 'w-32 h-32'} transform -rotate-90`}>
                         <circle cx={isMobile ? "40" : "64"} cy={isMobile ? "40" : "64"} r={isMobile ? "36" : "56"} stroke="currentColor" strokeWidth={isMobile ? "6" : "10"} fill="none" className="text-gray-200 dark:text-gray-700" />
                         <circle cx={isMobile ? "40" : "64"} cy={isMobile ? "40" : "64"} r={isMobile ? "36" : "56"} stroke="currentColor" strokeWidth={isMobile ? "6" : "10"} fill="none" strokeDasharray={`${2 * Math.PI * (isMobile ? 36 : 56)}`} strokeDashoffset={`${2 * Math.PI * (isMobile ? 36 : 56) * (1 - currentAnalysis.ats_score / 100)}`} className={`${getScoreRingColor(currentAnalysis.ats_score)} transition-all duration-1000`} />
                       </svg>
                       <div className="absolute inset-0 flex flex-col items-center justify-center">
                         <span className={`${isMobile ? 'text-lg' : 'text-3xl'} font-bold ${getScoreColor(currentAnalysis.ats_score)}`}>{currentAnalysis.ats_score}%</span>
                         <span className={`${isMobile ? 'text-[8px]' : 'text-xs'} text-gray-500`}>{t("score")}</span>
                       </div>
                      </div>
