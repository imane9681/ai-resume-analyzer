...
useEffect(() => {
  ...
  return (
    <div className={`min-h-screen p-4 md:p-8 pt-20 ${isMobile ? 'pt-16' : ''}`} dir={dir}>
      ...
      <div className={`max-w-7xl mx-auto ${isMobile ? 'mt-8' : 'mt-12'}`}>
        <div className={`text-center ${isMobile ? 'mb-8' : 'mb-12'}`}>
          <div className={`flex justify-center ${isMobile ? 'mb-4' : 'mb-6'}`}>
            <div className="relative">
              ...
              <div className={`relative bg-gradient-to-br from-green-500 to-emerald-600 ${isMobile ? 'p-3' : 'p-4'} rounded-2xl shadow-2xl animate-float`}>
                <Bot className={`${isMobile ? 'w-12 h-12' : 'w-16 h-16'} text-white`} />
              </div>
            </div>
          </div>
        ...
        <div className={`${isMobile ? 'mb-6' : 'mb-8'} max-w-4xl mx-auto ${isMobile ? 'px-3' : ''}`}>
          <ResumeDropzone 
        ...
        {isLoading && (
          <div className="mt-8 text-center">
            <div className={`inline-block animate-spin rounded-full ${isMobile ? 'h-10 w-10' : 'h-12 w-12'} border-b-2 border-green-600`}></div>
            <p className={`${isMobile ? 'mt-3' : 'mt-4'} text-gray-600 dark:text-gray-300 font-medium ${isMobile ? 'text-sm' : 'text-base'}`}>{t("analyzing")}</p>
            <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-gray-400 mt-1`}>{t("please_wait")}</p>
          </div>
        )}
        ...
        {currentAnalysis && !showCompare && !showDashboard && (
          <div className={`${isMobile ? 'mt-6' : 'mt-8'} max-w-5xl mx-auto ${isMobile ? 'px-3' : ''}`}>
            <div className={`${isMobile ? 'mb-6' : 'mb-8'}`}>
              <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
                <button
                  onClick={saveCurrentResumeLocal}
                  className={`flex items-center gap-1.5 sm:gap-2 cursor-pointer px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl border-2 border-emerald-500 text-emerald-600 dark:text-emerald-400 font-semibold ${isMobile ? 'text-xs' : 'text-sm'} transition-all duration-300`}
                >
                ...
                {user && (
                  <button
                    ...
                    className={`flex items-center gap-1.5 sm:gap-2 cursor-pointer px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white font-semibold ${isMobile ? 'text-xs' : 'text-sm'} shadow-md transition-all duration-300`}
                  >
                    <Upload className={`${isMobile ? 'w-3.5 h-3.5' : 'w-4 h-4'}`} />
                ...
                <button
                  ...
                  disabled={isSharing}
                  className={`flex items-center gap-1.5 sm:gap-2 cursor-pointer px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl bg-gradient-to-r from-emerald-800/80 to-green-700/80 hover:from-emerald-800/90 hover:to-green-700/90 text-white font-semibold ${isMobile ? 'text-xs' : 'text-sm'} shadow-md transition-all duration-300`}
                >
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 sm:gap-2 cursor-pointer px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl bg-gradient-to-r from-amber-600/70 to-orange-500/60 hover:from-amber-600/80 hover:to-orange-500/70 text-white font-semibold ${isMobile ? 'text-xs' : 'text-sm'} shadow-md transition-all duration-300`}
                >
                  <Lightbulb className={`${isMobile ? 'w-3.5 h-3.5' : 'w-4 h-4'}`} />
                  <span>{isImproving ? t("generating") : t("improve")}</span>
                </button>
              </div>
              
              <div className={`relative ${isMobile ? 'mt-4' : 'mt-6'}`}>
                <div className="absolute inset-0 flex items-center">
                  <div className="w-full border-t border-gray-200 dark:border-gray-700"></div>
                </div>
                <div className="relative flex justify-center">
                  <span className={`px-2 sm:px-3 py-0.5 sm:py-1 rounded-full ${isMobile ? 'text-[10px]' : 'text-xs'} text-gray-400 bg-white dark:bg-gray-800`}>{t("analysis_tools")}</span>
                </div>
              </div>
            </div>
            ...
            <div className={`bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border border-green-100 dark:border-green-900/30 ${isMobile ? 'mt-4' : 'mt-6'}`}>
              <div className={`bg-gradient-to-r from-green-600 via-emerald-600 to-green-600 text-white ${isMobile ? 'p-4' : 'p-6 md:p-8'}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className={`${isMobile ? 'text-xl' : 'text-2xl md:text-3xl'} font-bold`}>{t("analysis_report")}</h2>
                    <p className={`opacity-90 ${isMobile ? 'text-xs' : 'text-sm'} mt-0.5 sm:mt-1`}>{t("generated_by")}</p>
                  </div>
                  <Sparkles className={`${isMobile ? 'w-6 h-6' : 'w-8 h-8'} opacity-80`} />
                </div>
              </div>

              <div className={`${isMobile ? 'p-4' : 'p-6 md:p-8'} space-y-5 sm:space-y-8`}>
                <div className={`border-b-2 border-green-100 dark:border-green-900/30 ${isMobile ? 'pb-3' : 'pb-6'}`}>
                  <div className="flex items-center gap-1.5 sm:gap-2 text-green-600 dark:text-green-400 mb-1 sm:mb-2">
                    <Crown className={`${isMobile ? 'w-3.5 h-3.5' : 'w-4 h-4'}`} />
                    <span className={`${isMobile ? 'text-xs' : 'text-sm'} font-semibold uppercase tracking-wider`}>{t("candidate")}</span>
                  </div>
                  <p className={`${isMobile ? 'text-xl' : 'text-3xl md:text-4xl'} font-bold text-gray-800 dark:text-white break-words`}>{currentAnalysis.name}</p>
                </div>

                <div className="bg-gradient-to-br from-green-50 via-emerald-50 to-green-50 dark:from-green-900/20 dark:via-emerald-900/20 dark:to-green-900/20 rounded-2xl p-4 sm:p-6">
                  <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-4">
                    <Target className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} text-green-600 dark:text-green-400`} />
                    <h3 className={`${isMobile ? 'text-base' : 'text-lg'} font-semibold text-gray-800 dark:text-white`}>{t("ats_score")}</h3>
                  </div>
                  <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                    <div className={`relative ${isMobile ? 'w-24 h-24' : 'w-32 h-32'}`}>
                      <svg className={`${isMobile ? 'w-24 h-24' : 'w-32 h-32'} transform -rotate-90`}>
                        <circle cx={isMobile ? "48" : "64"} cy={isMobile ? "48" : "64"} r={isMobile ? "42" : "56"} stroke="currentColor" strokeWidth={isMobile ? "8" : "10"} fill="none" className="text-gray-200 dark:text-gray-700" />
                        <circle cx={isMobile ? "48" : "64"} cy={isMobile ? "48" : "64"} r={isMobile ? "42" : "56"} stroke="currentColor" strokeWidth={isMobile ? "8" : "10"} fill="none" strokeDasharray={`${2 * Math.PI * (isMobile ? 42 : 56)}`} strokeDashoffset={`${2 * Math.PI * (isMobile ? 42 : 56) * (1 - currentAnalysis.ats_score / 100)}`} className={`${getScoreRingColor(currentAnalysis.ats_score)} transition-all duration-1000`} />
                      </svg>
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className={`${isMobile ? 'text-xl' : 'text-3xl'} font-bold ${getScoreColor(currentAnalysis.ats_score)}`}>{currentAnalysis.ats_score}%</span>
                        <span className={`${isMobile ? 'text-[9px]' : 'text-xs'} text-gray-500`}>{t("score")}</span>
                      </div>
                    </div>
                    <div className="flex-1 w-full">
                      <div className={`${isMobile ? 'h-1.5' : 'h-2.5 sm:h-3'} bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden`}>
                        <div 
                      ...
                      <div className="flex items-center gap-1.5 sm:gap-2 mt-2 sm:mt-3">
                        {currentAnalysis.ats_score >= 80 ? (
                          <>
                            <Shield className={`${isMobile ? 'w-3.5 h-3.5' : 'w-4 h-4'} text-green-600`} />
                            <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-gray-700 dark:text-gray-300`}>{t("excellent")}</span>
                          </>
                        ) : currentAnalysis.ats_score >= 60 ? (
                          <>
                            <Zap className={`${isMobile ? 'w-3.5 h-3.5' : 'w-4 h-4'} text-emerald-600`} />
                            <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-gray-700 dark:text-gray-300`}>{t("good")}</span>
                          </>
                        ) : (
                          <>
                            <AlertTriangle className={`${isMobile ? 'w-3.5 h-3.5' : 'w-4 h-4'} text-orange-600`} />
                            <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-gray-700 dark:text-gray-300`}>{t("needs_improvement")}</span>
                          </>
                        )}
                      </div>
                    </div>
                ...
                {currentAnalysis.field && (
                  <div className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-2xl p-4 sm:p-6">
                    <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-3">
                      <Briefcase className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} text-purple-600 dark:text-purple-400`} />
                      <h3 className={`${isMobile ? 'text-base' : 'text-lg'} font-semibold text-gray-800 dark:text-white`}>{t("job_field")}</h3>
                    </div>
                    <p className={`${isMobile ? 'text-lg' : 'text-2xl'} font-bold text-purple-600 dark:text-purple-400 break-words`}>{currentAnalysis.field}</p>
                  </div>
                )}

                {currentAnalysis.job_match && (
                  <div className="bg-gradient-to-br from-teal-50 to-amber-50 dark:from-teal-900/20 dark:to-amber-900/20 rounded-2xl p-4 sm:p-6">
                    <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-4">
                      <Briefcase className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} text-teal-600 dark:text-teal-400`} />
                      <h3 className={`${isMobile ? 'text-base' : 'text-lg'} font-semibold text-gray-800 dark:text-white`}>{t("job_match")}</h3>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                      <div className={`relative ${isMobile ? 'w-24 h-24' : 'w-32 h-32'}`}>
                        <svg className={`${isMobile ? 'w-24 h-24' : 'w-32 h-32'} transform -rotate-90`}>
                          <circle cx={isMobile ? "48" : "64"} cy={isMobile ? "48" : "64"} r={isMobile ? "42" : "56"} stroke="currentColor" strokeWidth={isMobile ? "8" : "10"} fill="none" className="text-gray-200 dark:text-gray-700" />
                          <circle cx={isMobile ? "48" : "64"} cy={isMobile ? "48" : "64"} r={isMobile ? "42" : "56"} stroke="currentColor" strokeWidth={isMobile ? "8" : "10"} fill="none" strokeDasharray={`${2 * Math.PI * (isMobile ? 42 : 56)}`} strokeDashoffset={`${2 * Math.PI * (isMobile ? 42 : 56) * (1 - currentAnalysis.job_match.score / 100)}`} className="text-teal-500 transition-all duration-1000" />
                        </svg>
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <span className={`${isMobile ? 'text-xl' : 'text-3xl'} font-bold text-teal-600 dark:text-teal-400`}>{currentAnalysis.job_match.score}%</span>
                          <span className={`${isMobile ? 'text-[9px]' : 'text-xs'} text-gray-500`}>{currentAnalysis.job_match.matchLevel}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex-1">
                        ...
                        <div className={`${isMobile ? 'mt-3' : 'mt-4'}`}>
                          {currentAnalysis.job_match.matchedSkills.length > 0 && (
                            <>
                              <p className={`${isMobile ? 'text-xs' : 'text-sm'} font-medium text-gray-700 dark:text-gray-300 mb-1.5 sm:mb-2 flex items-center gap-1`}>
                                <CheckCircle className={`${isMobile ? 'w-3.5 h-3.5' : 'w-4 h-4'} text-green-500`} />
                              ...
                              <div className="flex flex-wrap gap-1 mb-2 sm:mb-3">
                                {currentAnalysis.job_match.matchedSkills.slice(0, isMobile ? 5 : 8).map((skill, i) => (
                                  <span key={i} className="bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 px-1.5 sm:px-2 py-0.5 sm:py-0.5 rounded-full ${isMobile ? 'text-[10px]' : 'text-xs'}">
                                    {skill}
                                  </span>
                                ))}
                              </div>
                          ...
                          {currentAnalysis.job_match.missingSkills.length > 0 && (
                            <>
                              <p className={`${isMobile ? 'text-xs' : 'text-sm'} font-medium text-gray-700 dark:text-gray-300 mb-1.5 sm:mb-2 flex items-center gap-1`}>
                                <AlertTriangle className={`${isMobile ? 'w-3.5 h-3.5' : 'w-4 h-4'} text-orange-500`} />
                              ...
                              <div className="flex flex-wrap gap-1">
                                {currentAnalysis.job_match.missingSkills.slice(0, isMobile ? 5 : 8).map((skill, i) => (
                                  <span key={i} className="bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-300 px-1.5 sm:px-2 py-0.5 sm:py-0.5 rounded-full ${isMobile ? 'text-[10px]' : 'text-xs'}">
                                    {skill}
                                  </span>
                                ))}
                              </div>
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-6">
                  <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-900/30 rounded-lg sm:rounded-xl p-3 sm:p-5 text-center group hover:scale-105 transition-transform duration-300">
                    <div className={`bg-green-600 ${isMobile ? 'w-8 h-8' : 'w-12 h-12'} rounded-lg sm:rounded-xl flex items-center justify-center mx-auto mb-1.5 sm:mb-3 shadow-lg`}>
                      <Briefcase className={`${isMobile ? 'w-4 h-4' : 'w-6 h-6'} text-white`} />
                    </div>
                    <p className={`${isMobile ? 'text-xl' : 'text-4xl'} font-bold text-green-600 dark:text-green-400`}>{currentAnalysis.years_experience}</p>
                    <p className={`${isMobile ? 'text-[10px]' : 'text-sm'} text-gray-600 dark:text-gray-400 mt-0.5 sm:mt-1`}>{t("years_experience")}</p>
                  </div>
                  <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-900/20 dark:to-emerald-900/30 rounded-lg sm:rounded-xl p-3 sm:p-5 text-center group hover:scale-105 transition-transform duration-300">
                    <div className={`bg-emerald-600 ${isMobile ? 'w-8 h-8' : 'w-12 h-12'} rounded-lg sm:rounded-xl flex items-center justify-center mx-auto mb-1.5 sm:mb-3 shadow-lg`}>
                      <GraduationCap className={`${isMobile ? 'w-4 h-4' : 'w-6 h-6'} text-white`} />
                    </div>
                    <p className={`${isMobile ? 'text-xs' : 'text-xl'} font-bold text-emerald-600 dark:text-emerald-400 break-words line-clamp-2`}>{currentAnalysis.education}</p>
                    <p className={`${isMobile ? 'text-[10px]' : 'text-sm'} text-gray-600 dark:text-gray-400 mt-0.5 sm:mt-1`}>{t("education")}</p>
                  </div>
                </div>

                <div>
                  <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-4">
                    <Code2 className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} text-green-600 dark:text-green-400`} />
                    <h3 className={`${isMobile ? 'text-base' : 'text-lg'} font-semibold text-gray-800 dark:text-white`}>{t("skills")}</h3>
                  </div>
                  <div className="flex flex-wrap gap-1.5 sm:gap-2">
                    {currentAnalysis.skills.slice(0, isMobile ? 10 : currentAnalysis.skills.length).map((skill, i) => (
                      <span key={i} className={`bg-gradient-to-r from-green-100 to-emerald-100 dark:from-green-900/40 dark:to-emerald-900/40 text-green-800 dark:text-green-300 ${isMobile ? 'px-2 py-1 text-[11px]' : 'px-4 py-2 text-sm'} rounded-full font-medium border border-green-200 dark:border-green-800 hover:scale-105 transition-transform cursor-default`}>
                        {skill}
                      </span>
                    ))}
                    {currentAnalysis.skills.length > 10 && isMobile && (
                      <span className="bg-gradient-to-r from-green-100 to-emerald-100 dark:from-green-900/40 dark:to-emerald-900/40 text-green-800 dark:text-green-300 px-2 py-1 text-[11px] rounded-full font-medium border border-green-200 dark:border-green-800">
                        +{currentAnalysis.skills.length - 10}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                  <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-900/30 rounded-lg sm:rounded-xl p-4 sm:p-5">
                    <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
                      <Award className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} text-green-600`} />
                      <h3 className={`font-semibold text-gray-800 dark:text-white ${isMobile ? 'text-sm' : 'text-base'}`}>{t("strengths")}</h3>
                    </div>
                    <ul className="space-y-1.5 sm:space-y-2">
                      {currentAnalysis.strengths.slice(0, isMobile ? 3 : currentAnalysis.strengths.length).map((strength, i) => (
                        <li key={i} className="flex items-start gap-1.5 sm:gap-2">
                          <Star className={`${isMobile ? 'w-3 h-3' : 'w-4 h-4'} text-green-500 mt-0.5 flex-shrink-0`} />
                          <span className={`text-gray-700 dark:text-gray-300 ${isMobile ? 'text-xs' : 'text-sm'} break-words`}>{strength}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-900/30 rounded-lg sm:rounded-xl p-4 sm:p-5">
                    <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
                      <AlertTriangle className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} text-orange-600`} />
                      <h3 className={`font-semibold text-gray-800 dark:text-white ${isMobile ? 'text-sm' : 'text-base'}`}>{t("weaknesses")}</h3>
                    </div>
                    <ul className="space-y-1.5 sm:space-y-2">
                      {currentAnalysis.weaknesses.slice(0, isMobile ? 3 : currentAnalysis.weaknesses.length).map((weakness, i) => (
                        <li key={i} className="flex items-start gap-1.5 sm:gap-2">
                          <div className={`${isMobile ? 'w-1 h-1' : 'w-1.5 h-1.5'} bg-orange-500 rounded-full mt-1.5 sm:mt-2 flex-shrink-0`}></div>
                          <span className={`text-gray-700 dark:text-gray-300 ${isMobile ? 'text-xs' : 'text-sm'} break-words`}>{weakness}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <div className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-900/30 rounded-lg sm:rounded-xl p-4 sm:p-5">
                  <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-4">
                    <Lightbulb className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} text-purple-600`} />
                    <h3 className={`${isMobile ? 'text-base' : 'text-lg'} font-semibold text-gray-800 dark:text-white`}>{t("recommendations")}</h3>
                  </div>
                  <ul className="space-y-2 sm:space-y-3">
                    {currentAnalysis.recommendations.map((rec, i) => (
                      <li key={i} className="flex items-start gap-2 sm:gap-3">
                        <div className={`bg-purple-600 ${isMobile ? 'w-4 h-4' : 'w-5 h-5'} rounded-full flex items-center justify-center flex-shrink-0 mt-0.5`}>
                          <span className={`text-white ${isMobile ? 'text-[10px]' : 'text-xs'} font-bold`}>{i + 1}</span>
                        </div>
                        <span className={`text-gray-700 dark:text-gray-300 ${isMobile ? 'text-xs' : 'text-sm'} break-words flex-1`}>{rec}</span>
                      </li>
                    ))}
                  </ul>
                </div>

                <div className={`text-center ${isMobile ? 'text-[10px]' : 'text-xs'} text-gray-400 dark:text-gray-500 ${isMobile ? 'pt-3' : 'pt-4'} border-t border-gray-200 dark:border-gray-700`}>
                  <LayoutDashboard className={`${isMobile ? 'w-2.5 h-2.5' : 'w-3 h-3'} inline mr-1`} />
          ...
          <div className="mt-16 max-w-6xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className={`${isMobile ? 'text-lg' : 'text-xl'} font-bold text-gray-800 dark:text-white`}>
                    {t("saved_resumes")}
                  </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {savedResumes.map((resume, idx) => (
                ...
                >
                  ...
                  <div className={`${isMobile ? 'p-3' : 'p-4'} pb-2`}>
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2">
                        <div className={`${isMobile ? 'w-8 h-8' : 'w-10 h-10'} rounded-xl bg-gradient-to-br from-green-100 to-emerald-100 dark:from-green-900/40 dark:to-emerald-900/40 flex items-center justify-center`}>
                          <FileText className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} text-green-700 dark:text-green-400`} />
                        </div>
                        <div>
                  ...
                  <div className={`${isMobile ? 'p-3' : 'p-4'}`}>
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {resume.analysis.skills.slice(0, isMobile ? 3 : 4).map((skill, i) => (
                        <span key={i} className="px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 text-[10px] font-medium truncate max-w-[80px]">
                          {skill}
                        </span>
                      ))}
